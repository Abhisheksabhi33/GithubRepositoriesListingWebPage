document.addEventListener("DOMContentLoaded", function () {
  ...
  function filterRepositories() {
    ...
    const repositories = document
      .getElementById("repositoryGrid")
      .getElementsByClassName("col");

    for (let i = 0; i < repositories.length; i++) {
      const repoName = repositories[i].getElementsByClassName("repo-name")[0];
      if (repoName) {
        const repoNameText = repoName.innerText.toLowerCase();
        const shouldDisplay = repoNameText.includes(input);
        repositories[i].style.display = shouldDisplay ? "" : "none";
      }
    }
  }

  // Attach filterRepositories function to the search button click event
  document
  ...
  // Attach filterRepositories function to the keyup event on the search input
